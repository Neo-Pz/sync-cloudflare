...
export class TldrawDurableObject {
	...
	private readonly router = AutoRouter({
		...
		// 存储发布快照（开发模式）
		.post('/store-snapshot/:publishedSlug', async (request) => {
			try {
				const { publishedSlug } = request.params
				const publishSnapshot = await request.json()
				
				await this.ctx.storage.put(`publish-snapshot:${publishedSlug}`, publishSnapshot)
				console.log('✅ 发布快照已存储到Durable Object:', publishedSlug)
				
				return new Response(JSON.stringify({ success: true }), {
					headers: { 'Content-Type': 'application/json' }
				})
			} catch (error) {
				console.error('❌ 存储发布快照失败:', error)
		...
		// 获取发布快照（开发模式）
		.get('/get-snapshot/:publishedSlug', async (request) => {
			try {
				const { publishedSlug } = request.params
				const publishSnapshot = await this.ctx.storage.get(`publish-snapshot:${publishedSlug}`)
				
				if (publishSnapshot) {
					console.log('✅ 从Durable Object加载发布快照:', publishedSlug)
					return new Response(JSON.stringify(publishSnapshot), {
						headers: { 'Content-Type': 'application/json' }
					})
				} else {
					console.log('⚠️ Durable Object中未找到发布快照:', publishedSlug)
					return new Response(JSON.stringify({ error: '发布快照未找到' }), {
						status: 404,
						headers: { 'Content-Type': 'application/json' }
					})
				}
